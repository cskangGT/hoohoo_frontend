...
import { TouchableOpacity, View, FlatList, TextInput, ImageBackground, Text, Platform } from 'react-native';
...
const ListView = ({ navigation, route }: any) => {
  ...
  const toggleItemSelection = (item: ItemData) => {
    if (item === null) {
      return;
    }
  ...
  const toggleSelection = () => setIsSelectable(!isSelectable);
  const done = () => {
    toggleSelection();
    setExportData([]);
  }
